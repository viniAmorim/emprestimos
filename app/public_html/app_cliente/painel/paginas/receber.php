...
//totalizar páginas
if ($pago == 'Vencidas') {
  $query2 = $pdo->query("SELECT * from $pag where data_venc < curDate() and pago != 'Sim' and cliente = '$id_usuario' order by id desc");
} else {
  $query2 = $pdo->query("SELECT * from $pag where cliente = '$id_usuario' and pago LIKE '%$pago%' order by id desc");
}
...
<form method="post" style="display: none;">
  ...
  <div class="card card-style">
    <div class="content">
      <?php
      if ($pago == 'Vencidas') {
        $query = $pdo->query("SELECT * from $pag where data_venc < curDate() and pago != 'Sim' and cliente = '$id_usuario' order by id desc");
      } else {
        $query = $pdo->query("SELECT * from $pag where pago LIKE '%$pago%' and cliente = '$id_usuario' order by id desc ");
      }
      $valor_pago = 0;
      $valor_pendentes = 0;
      ...
      <div class="splide__slide mx-3">
        <div class="d-flex">
          ...
          <div class="ms-auto">            
           ...
           <!-- Botão Arquivo -->
<a onclick="arquivo('{$id}', '{$descricao}')" href="#" class="icon icon-xs rounded-circle shadow-sm bg-dark" title="Abrir Arquivo">
  <i class="fa fa-file text-white"></i>
</a>

<!-- Botão Pagamento -->
<a href="../../pagar/{$id}" target="_blank" class="icon icon-xs rounded-circle shadow-sm bg-success {$ocultar}" title="Efetuar Pagamento">
  <i class="fa fa-usd text-white"></i>
</a>

<!-- Botão Recibo -->
<form method="POST" action="../../painel/rel/recibo_class.php" style="display:inline-block">
  <input type="hidden" name="id" value="{$id}">
  <input type="hidden" name="enviar" value="Não">
  <button type="submit" class="icon icon-xs rounded-circle shadow-sm bg-danger {$ocultar_recibo}" title="Abrir Recibo" style="border:none;">
