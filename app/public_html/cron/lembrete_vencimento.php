...
$query = $pdo->query("SELECT * from receber where data_venc = '$data_lembrete' and pago != 'Sim' and (alerta_lembrete is null or alerta_lembrete != 'Sim') and cliente > 0 and cliente is not null and hora_alerta <= curTime() ");
...
if($bloquear_disparos != "Sim"){
	require('texto.php');
		...
		if(@$status_mensagem == "Mensagem enviada com sucesso." and $seletor_api == 'menuia'){
			$pdo->query("UPDATE receber SET alerta_lembrete = 'Sim' where id = '$id'");
		}
