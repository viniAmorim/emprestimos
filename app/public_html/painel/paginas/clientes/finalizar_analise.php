...
// Garante que erros de PHP sejam exibidos para depuração
...
// Em caso de extensão não permitida no upload, retorna erro JSON em vez de morrer com die()
returnJson('erro', "Extensão de arquivo para '{$prefix}' não permitida! (Permitidas: " . implode(', ', $allowed_extensions) . ')');
}
} else {
...
// CORREÇÃO: Retorna JSON em vez de die() simples
returnJson('erro', "Erro: ID do cliente ou status final não fornecido.");
}

 // --- RESTRIÇÃO DE DUPLICIDADE (RETORNA JSON AQUI) ---
 if ($status_final === 'Aprovado') {
$sql_alerta = "SELECT COUNT(*) FROM alertas_duplicidade WHERE id_cliente_cadastrado = :id_cliente AND resolvido = 0";
$query_alerta = $pdo->prepare($sql_alerta);
$query_alerta->bindValue(":id_cliente", $id_cliente);
$query_alerta->execute();
$alertas_nao_resolvidos = $query_alerta->fetchColumn();

if ($alertas_nao_resolvidos > 0) {
$mensagem_erro = "Não foi possível aprovar o cliente! Existem **{$alertas_nao_resolvidos} alertas de duplicidade não resolvidos** para este cliente. Por favor, resolva-os antes de aprovar.";

// Retorna o JSON de erro e encerra a execução.
returnJson('erro', $mensagem_erro); 
}
 }

// Define qual campo de observação preenche a coluna 'observacoes' (geral/pendente/aprovado)
$observacoes = null;
// OBSERVAÇÃO CRÍTICA: Se o status for Reprovado, é mais comum que a observação principal seja a de reprovação.
// Ajustei a lógica de $observacoes para priorizar as observações específicas (Reprovado/Pendente)
$observacoes_reprovacao_valor = trim($_POST['observacoes_reprovacao'] ?? '');

if ($status_final === 'Reprovado') {
// Reprovado usa a coluna observacoes_reprovacao para o motivo
$observacoes = $_POST['observacoes'] ?? null; // Usa o campo de observação geral/motivo_reprovacao
} elseif ($status_final === 'Pendente') {
$observacoes = $_POST['observacoes_pendente_desc'] ?? null;
} else { // 'Aprovado' ou outro
$observacoes = $_POST['observacoes_genericas_desc'] ?? null;
}

// Se o campo de observacao_reprovacao_valor for preenchido, ele será usado na query
// A variável $observacoes continuará a ser usada para a coluna observacoes geral.
...
// Removida a linha: $observacoes = trim($_POST['observacoes_reprovacao'] ?? '');
...
// NOTA: A função processUpload agora retorna JSON se a extensão for inválida, eliminando die() simples.
...
status = :status_final, 
observacoes_reprovacao = :observacoes_reprovacao_valor, 
...
// BIND CORRIGIDO: Usa a nova variável para reprovação
$query_update_cliente->bindValue(":observacoes_reprovacao_valor", $observacoes_reprovacao_valor);
// BIND CORRIGIDO: Usa a variável $observacoes que contém o valor geral/pendente/aprovado
...
// --- CORREÇÃO CRÍTICA: RETORNAR JSON DE SUCESSO EM VEZ DE EXIT() ---
$mensagem_sucesso = "Análise finalizada com sucesso! Status: {$status_final}.";
returnJson('sucesso', $mensagem_sucesso); 

} else {
// --- CORREÇÃO: RETORNAR JSON DE ERRO PARA MÉTODO INVÁLIDO ---
returnJson('erro', "Método de requisição inválido. Apenas POST é permitido.");
}
?>